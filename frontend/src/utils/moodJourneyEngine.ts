...
// Accumulate every chosen option's axis nudges into one raw vector.
...
// Normalize the raw accumulated vector to -1..1 so it's comparable to the
// mood_vector shape other games already send (see GameSignals.moodVector).
...
// Nearest-cluster resolution with a deterministic tie-break, so the journey
// always lands on ONE confident answer — never a "you're between two moods"
// screen. Tie-break order: (1) the axis with the largest magnitude in the
// user's vector, (2) the twist scene's cluster hint, (3) declaration order.
